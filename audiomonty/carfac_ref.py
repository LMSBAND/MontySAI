# Vendored from google/carfac python/src/carfac/np/carfac.py (Apache-2.0),
# with np.concat -> np.concatenate for older numpy. This is the same
# reference lms_carfac.jsfx was verified against; the ear rig runs audio
# through THIS and treats the result as ground-truth NAP.
#
...
def shift_right_two(s: np.ndarray) -> np.ndarray:
  ...
  Returns:
    The shifted vector.
  """
  return np.concatenate((s[0, np.newaxis, ...], s[1, np.newaxis, ...], s[:-2, ...]))
...
def shift_right_one(s: np.ndarray) -> np.ndarray:
  ...
  Returns:
    The shifted vector.
  """
  return np.concatenate((s[0, np.newaxis, ...], s[:-1, ...]))
...
def shift_left_one(s: np.ndarray) -> np.ndarray:
  ...
  Returns:
    The shifted vector.
  """
  return np.concatenate((s[1:, ...], s[-1, np.newaxis, ...]))
...
def shift_left_two(s: np.ndarray) -> np.ndarray:
  ...
  Returns:
    The shifted vector.
  """
  return np.concatenate((s[2:, ...], s[-1, np.newaxis, ...], s[-2, np.newaxis, ...]))
